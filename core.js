...
import a3_0x3c67ec from './logger.js';
import { RPC } from './rpc.js';
import { SONICLABS } from './dapps.js';

export default class Core extends API {
  constructor(_0x169d4e, _0x166cad, _0x1aa4a6) {
    super("https://airdrop.soniclabs.com", _0x1aa4a6, 'airdrop.soniclabs.com', 'https://airdrop.soniclabs.com/');
    this.acc = _0x169d4e;
    this.sessionId = 0x1;
  ...
  async ["connectSonicDapps"]() {
    ...
    const _0x184203 = "I'm joining Sonic Airdrop Dashboard with my wallet, and I agree to the terms and conditions.\nWallet address:\n" + this.address + "\n";
    ...
    if (!_0x20b7c7.error) {
      ...
      if (_0x20b7c7.error?.["message"]?.["includes"]("limit")) {
        ...
        if (_0x20b7c7.error?.['message']?.["includes"]("random number")) {
          await Helper.delay(30000, this.acc, "Failed to Play Plinko Game - " + _0x20b7c7.error?.["message"], this);
        } else {
  ...
  async ["playWheel"]() {
    ...
    this.sessionId += 0x1;
    if (!_0x1f49c3.error) {
      await Helper.delay(30000, this.acc, "Successfully Play Whell Game", this);
    } else {
    ...
    if (_0x5b7964.error) {
      if (_0x5b7964.error?.["message"]?.["includes"]("limit")) {
        ...
        if (_0x5b7964.error?.["message"]?.['includes']("random number")) {
          await Helper.delay(30000, this.acc, "Failed to Play Mine Game - " + _0x5b7964.error?.["message"], this);
          return;
        } else {
